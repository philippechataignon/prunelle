...
matflux::matflux(int w_dim, int w_nbval):dim(w_dim), nbval(w_nbval), indlig(0),
indcol(0), indval(0)
{
    // tableau des valeurs
...
element *matflux::ajoute_element(element * vtete[], int ind, element * pins,
                                 element * pp)
{
...
element *matflux::insere_element(element * vtete[], int ind, element * pins,
...
element *matflux::delete_element(element * vtete[], int ind, element * p,
                                 element * pp)
{
...
int matflux::insert(element * vtete[], int ind, element * pins)
{
...
void matflux::merge_lc(element * vtete_l[], element * vtete_c[], int inda,
                       int indb)
{
...
void matflux::change_ind(element * vtete[], int ind, int indb, int inda)
{
    // Transforme indb en inda dans liste ind
    for (element * p = vtete[ind], *pp = 0; p != 0 && p->numlc >= indb;
         pp = p, p = p->next) {
        if (p->numlc == indb) {
            p->numlc = inda;
...
void matflux::delete_ind(element * vtete[], int ind, int indb)
{

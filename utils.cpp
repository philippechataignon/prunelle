...
void lecture_fich(std::istream & in, matflux & mflux,
...
void agrege(matflux & mflux, std::vector < commune > &vcom, int numdca,
            int numdcb)
{
    // std::cout << "Agrégation" << std::endl ;
    // migrants internes = flux ab + flux ba
    double mi =
        mflux.get_val(numdca, numdcb).nb + mflux.get_val(numdcb, numdca).nb;

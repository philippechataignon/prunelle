...
int main(int argc, char *argv[])
{
    ...
    if (!flu.is_open()) {
        std::cerr << "Erreur : impossible d'ouvrir le fichier couple" << std::
            endl;
        std::exit(1);
    }
    ...
    if (!out_flu.is_open()) {
        std::cerr << "Erreur : impossible d'ouvrir le fichier sortie" << std::
            endl;
        std::exit(2);
    }
    ...
    nbflux--;
    std::cerr << "Nb flux non nuls : " << nbflux << " - Taille : " << set_com.
        size() << std::endl;
    ...
    int i = 0;
    for (std::set < std::string >::iterator p = set_com.begin();
         p != set_com.end(); p++) {
        out_flu << i << "\t" << *p << "\n";
        i++;
    }
    ...
    while (flu) {
        ...
        if (!coder.empty() && nb != 0) {
            out_flu << std::distance(vcom.begin(),
                                     lower_bound(vcom.begin(), vcom.end(),
                                                 coder))
                << "\t" << std::distance(vcom.begin(),
                                         lower_bound(vcom.begin(), vcom.end(),
                                                     codelt))
                << "\t" << nb << "\n";
            ;

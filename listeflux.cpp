...
flux *
listeflux::ajoute_flux (int numdc, flux * pins, flux * pp)
{
        ...
        case 3 :
            {
                ...
                int act0 = vcom[numdcr].act;  
                int act1 = vcom[numdclt].act;
                int sta0 = vcom[numdcr].sta;  
                int sta1 = vcom[numdclt].sta;
                int staab = sta0+sta1+rech_flux(numdclt,numdcr)+p->nb ;
                if ( act0 !=0 && act1 !=0 ) {
                    float txsta0 = static_cast<float> (sta0)/act0 ;
                    float txsta  = static_cast<float> (sta1)/act1 ;
                    if (txsta0 > txsta) {
                        txsta = txsta0 ;
                    }
                    p->lien = static_cast<float>( staab ) / (act0+act1) - txsta ;
                } else {
                    p->lien = 0;
                }
                break;
...
listeflux::merge_flux (std::vector < commune > &vcom , int numdca, int numdcb , int type )
{
    ...
    while (pa != 0 || pb != 0) {
        if ((pa != 0) && (pb == 0 || pa->numdclt > pb->numdclt)) {
            ...
            pa = pa->next;
        } else if ((pb != 0) && (pa == 0 || pa->numdclt < pb->numdclt)) {

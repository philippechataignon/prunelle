...
int main(int argc, char *argv[])
{
    ...
    std::cin >> nbflux;
    std::cin >> nbcom;
    std::cerr << nbflux << "/" << nbcom << "/" << opt.
        get_typelien() << "/" << opt.get_verbeux() << std::endl;
    ...
    if (opt.get_pre().length() > 0) {
        ...
        while (!pre.eof()) {
            int sat, pole;
            pre >> pole >> sat;
            out << vcom[pole].nom << "\t" << vcom[sat].
                nom << "\t" << -1 << "\t" << -1 << "\n";
            if (opt.get_verbeux() >= 1) {
                std::cerr << "PRE: " << vcom[pole].nom << "<" << vcom[sat].
                    nom << "\n";
            }
    ...
    while (!fin && cpt++ < (nbcom - 1)) {
        ...
        for (int i = 0; i < nbcom; i++) {
            if (vcom[i].status &&
                (vcom[i].maxlien > maxlien ||
                 (sat != -1 && vcom[i].maxlien == maxlien
                  && vcom[i].act < vcom[sat].act)
                )
                ) {
                sat = i;
        ...
        if (!fin) {
            ...
            out << vcom[pole].nom << "\t" << vcom[sat].
                nom << "\t" << maxlien << "\t" << rebond << "\n";
            if (opt.get_verbeux() >= 1) {
                std::cerr << "AGR: " << vcom[pole].nom << "<" << vcom[sat].
                    nom << '\t' << maxlien << "\t" << rebond << "\n";
            }
            agrege(mflux, vcom, pole, sat);
            vlien->raz_calc(pole);
